...
int ancestors(struct node *root, struct node *node)
{
    if (root == NULL)
    {
        return 0;
    }
    if (root->left == node || root->right == node || ancestors(root->left, root) || ancestors(root->right, node))
    {
        printf("%d ", root->data);
        return 1;
    }
...
int main()
{
    ...
    scanf("%d", &data);
    struct node *new = find(root, data);
    printf("The ancestors are:\n");
    ancestors(root, new);
    printf("\n");
    return 0;
}