...
int isom(struct node *root, struct node *shoot)
{
    ...
    {
        return (isom(root->left, shoot->left) && isom(root->right, shoot->right));
    }
}
int main()
{
    ...
    if (isom(root, shoot))
    {
        printf("The two trees are isomorphic each other\n");
    }
    else
    {
        printf("The two trees are not isomorphic to each other\n");
    }
    return 0;
}